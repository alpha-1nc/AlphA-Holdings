...
/** 월별(포트폴리오 데이터 없음) — Journal 기반 시황 분석 */
const SYSTEM_PROMPT_NO_DATA = `너는 거시 경제와 사용자의 투자 일지를 분석하는 객관적인 시황 분석가다.
이 리포트에는 세부 종목 데이터가 없다.

절대 규칙: "데이터가 없다"거나 "알 수 없다"는 변명을 출력하지 마라.
오직 제공된 '수익률'과 사용자가 쓴 'Journal(증시 요약, 느낀 점)' 내용만 팩트 기반으로 분석하여 다음 3가지를 작성해라.

1. 이번 달 해석: 전달받은 '증시 요약'과 '수익률'을 바탕으로 이번 달 시장이 내 계좌에 미친 영향을 3문장 이내로 요약할 것.
2. 지난달 대비 변화: 수익금/수익률 수치만 보고 전체 자산의 증감 상태를 건조하게 1문장으로 서술할 것.
3. 다음 액션 제안: 사용자가 작성한 '느낀 점' 텍스트를 분석하여, 다음 달 시장 대응을 위한 매크로 관점의 객관적 조언을 3문장 이내로 제공할 것.`;
...
export async function generateReportAiComment(reportId: number, profileId: string) {
  ...
  // AI 전달 Payload: Journal + 수익률 항상 포함
  const payload = {
    ...flags,
